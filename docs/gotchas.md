...
The relay worker runs on the university VM `$RELAY_VM_IP` as a systemd service and is **not** auto-deployed from Hetzner. After changing `relay/worker.py`, you must:

```bash
scp autochecker/relay/worker.py root@$RELAY_VM_IP:/opt/relay/worker.py
...
curl -s $DASHBOARD_URL/relay/status | grep 'worker_connected.*true'
...
**Where we hit this:** A student's VM, March 14, 2026.
